...
# https://github.com/bhataprameya/zsh-install
#
# One-liner:
...
: "${ASSET_BASE:=https://raw.githubusercontent.com/bhataprameya/zsh-install/main}"
: "${NO_CHSH:=0}"
: "${NO_FONT:=0}"
: "${NO_ZSH_INSTALL:=0}"
...
# --------------------------------------------------------------------------- #
# 5. Generate a clean, portable .zshrc
# --------------------------------------------------------------------------- #
log "Writing clean ~/.zshrc"
backup "$HOME/.zshrc"
cat > "$HOME/.zshrc" <<'ZSHRC'
# ~/.zshrc — generated by zsh-install (github.com/bhataprameya/zsh-install)
...
if [ "${ALL_USERS:-0}" = "1" ]; then
  ...
  if [ "$(id -u)" -eq 0 ] || [ -n "$SUDO" ]; then
    ...
    provision_user() { # $1=user  $2=home
      ...
      # back up whatever they had, then install our configs
      ...
      $SUDO cp "$HOME/.zshrc"    "$_uh/.zshrc"    2>/dev/null || return 1
